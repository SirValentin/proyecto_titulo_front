<template>
  <div>
    <DataTable :value="items" :stripedRows="true" tableStyle="min-width: 100%">
      <template #header>
        <div class="flex space-x-2">
          <Button
            label="Crear empleado"
            class="bg-skin-primary-60 text-xl text-white py-2 px-4"
            @click="formCrearEmpleadoModal = true"
          />
        </div>
      </template>
      <Column
    ...
    <CrearEmpleadoModal
      v-if="formCrearEmpleadoModal"
      :visible="formCrearEmpleadoModal"
      @cerrar="formCrearEmpleadoModal = false"
    />
  </div>
</template>
...
const { getSucursales: listaSucursales } = storeToRefs(storeSucursal);
const { getCargos: listaCargos } = storeToRefs(storeCargo);
...
const items = computed(() => {
  return getEmpleados.value.map((empleado) => {
    const sucursal = listaSucursales.value.find(
      (sucursal) => sucursal.id === empleado.sucursal
    );
    const cargo = listaCargos.value.find(
      (cargo) => cargo.id === empleado.cargo
    );
